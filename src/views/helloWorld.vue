<template lang="pug">
    v-col(cols="12" sm="8")
      ...
      v-card(min-height="80vh" rounded="lg")
        v-card-title 開倉建議計算
        v-card-text
          v-container
            ...
            h3 總資金 ：{{thousandComma(calculate.TA)}}
            br
            h3 停損比率(百分比) ：{{this.calculate.StopLoss}} %
            br 
            h3 操作方向 ： {{LongShort}}
            br
            h3 {{inTitle}} ： {{thousandComma(calculate.in)}}
            br
            h3 {{outTitle}} ： {{thousandComma(calculate.out)}}
            br
            h3 建議開倉位大小 ： {{thousandComma(Long_Open)}}
